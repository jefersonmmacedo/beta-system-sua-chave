...
export function DeleteProposal({id, idProperty, nameClient, cpfCnpjClient}) {
    const Local = localStorage.getItem("adm-suachave");
    const user = JSON.parse(Local);

    const { deleteProposalCompany } = useContext(AuthContext)
    ...
    return (
       ...
       <ReactTooltip id='Deletar' place="bottom" type="dark" effect="solid">
        ...
        <div className="itensModal-Process">
          <div className="textModalProposal">
                  <IoCloseCircle />
                  <h3>Deseja deletar a proposta:</h3>
                  <h4>Contrato: {id}</h4>
                  <h4>Imóvel: {title}</h4>
                  <h5>{type} - {subType}</h5>
                  <br />
            ...
            <div className="buttons">
                    <button onClick={handleDeleteProposal}><IoTrashOutline/> Deletar</button>
