...
export function EditContract() {
    ...
    const [newContract, setnewContract] = useState("Novo");
    ...
    const [proportionalRent, setProportionalRent] = useState("");
    ...
    useEffect(() => {
        async function loadContract() {
            await api.get(`/contracts/${id}`).then((res) => {
                ...
                setIdLocator(res.data[0]?.idLocator)
                setNameLocator(res.data[0]?.nameLocator)
                setCpfCnpjLocator(res.data[0]?.cpfCnpjLocator)
                setIdClient(res.data[0]?.idClient)
                setNameClient(res.data[0]?.nameClient)
                setCpfCnpjClient(res.data[0]?.cpfCnpjClient)
                setIdGuarantor(res.data[0]?.idGuarantor)
                setNameGuarantor(res.data[0]?.nameGuarantor)
                setCpfCnpjGuarantor(res.data[0]?.cpfCnpjGuarantorj)
                setValueContract(res.data[0]?.valueContract)
                setnewContract(res.data[0]?.newContract)
                setMaturityContract(res.data[0]?.maturityContract)
                setCondominium(res.data[0]?.condominium)
                setIptu(res.data[0]?.iptu)
                setOtherPrices(res.data[0]?.otherPrices)
                setAssurance(res.data[0]?.assurance)
                setSecurityDeposit(res.data[0]?.securityDeposit)
                setStartContract(new Date(res.data[0]?.startContract));
                setParcels(parseInt(res.data[0]?.parcels));
                setAdjustment(res.data[0]?.adjustment);
                setProportionalRent(res.data[0]?.proportionalRent)
                setDaysProportional(parseInt(res.data[0]?.daysProportional))
                setTransfer(res.data[0]?.transfer)
                setTransferAmount(res.data[0]?.transferAmount)
                setFireInsurance(res.data[0]?.fireInsurance)
                setValueFireInsurance(res.data[0]?.valueFireInsurance)
                setPercentage(parseInt(res.data[0]?.percentage))
            }).catch((error) => {
                console.log(error)
            })
        }
        ...
        <div className="EditContract">
            ...
            <h3>Novo contrato</h3>
                ...
                <div className="dataInfo">
                    ...
                    <select value={newContract} onChange={handleNewContract}>
                        <option value="">Escolha</option>
            ...
            <h4>Duração de contrato</h4>
                    ...
                    <input type="text" disabled className={startContract === "" ? "" : "select"} value={startContract?.toLocaleDateString()} onChange={e => setStartContract(e.target.value)}/>
                    </div>
                    <div className="dataInfo">
                    ...
                    <input type="text" disabled className={endContract === "" ? "" : "select"} value={endContract.toLocaleDateString()}/>
                    </div>
                    <div className="dataInfo">
                  ...
                  <div className="dataInfo">
                        ...
                        <input type="text" className={readjustedRentDate === "" ? "" : "select"}
                             disabled   value={readjustedRentDate.toLocaleDateString()}/>
                        </div>}
                        {adjustment !== "Sim" ? "" : 
                  <div className="dataInfo">
            ...
            <div className="newInfo">
                    ...
                    <button className="btnSendContract" onClick={handleEditContract}>Cadastrar novo contrato</button>
                    </div>
            </div>
        </div>
    )
